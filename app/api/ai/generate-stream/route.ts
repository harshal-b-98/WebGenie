...
// Cache widget files for inline injection (avoids repeated file reads)
let widgetCSSCache: string | null = null;
let widgetJSCache: string | null = null;

function getWidgetCSS(): string {
  if (!widgetCSSCache) {
    try {
      const cssPath = path.join(process.cwd(), "public/chat-widget/widget.css");
      widgetCSSCache = fs.readFileSync(cssPath, "utf-8");
    } catch (error) {
      logger.error("Failed to read widget CSS", error);
      widgetCSSCache = "/* Widget CSS failed to load */";
    }
  }
  return widgetCSSCache;
}

function getWidgetJS(): string {
  if (!widgetJSCache) {
    try {
      const jsPath = path.join(process.cwd(), "public/chat-widget/widget.js");
      widgetJSCache = fs.readFileSync(jsPath, "utf-8");
    } catch (error) {
      logger.error("Failed to read widget JS", error);
      widgetJSCache = "/* Widget JS failed to load */";
    }
  }
  return widgetJSCache;
}

// Helper to inject chat widget - INLINE CSS/JS to work in blob URL iframes
function injectChatWidget(
  ...
  // INLINE the CSS and JS instead of loading from external URLs
  // This is necessary because blob URL iframes cannot load external resources
  const widgetCode = `
